...
function removeLastOccurrences(str, value) {
  ...
  return i >= 0 ? str.slice(0, i) + str.slice(i + value.length) : str;
}
...
function isPalindrome(str) {
  const newStr = str.replace(/[ .,?!]/g, '').toUpperCase();
...
function reverseWords(str) {
  ...
  let resultStr = '';
  for (let i = 0; i < arr.length; i += 1) {
    const word = arr[i].split('').reverse().join('');
    resultStr = resultStr.concat(' ', word);
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const letter = str[i];
    const finalLetter = initial.includes(letter)
      ? final[initial.indexOf(letter)]
      : letter;
...
function getCardId(value) {
  ...
  return (
    cardValues.indexOf(value[0]) +
    cardSuits.indexOf(value[value.length - 1]) * 13
  );
}
